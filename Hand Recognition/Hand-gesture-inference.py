...
def segment(frame, threshold=20):
    ...
    if len(contours) == 0:
        ...
        # In newer OpenCV versions, hierarchy is not returned
        if hierarchy is None:
            hierarchy = [None]
        hand_segment = max(contours, key=cv2.contourArea)
        return (thresholded, hand_segment, hierarchy)
...
while True:
    ...
    if num_frames < 60:
        ...
        if hand is not None:
            if len(hand) == 2:  # Old OpenCV version
                thresholded, hand_segment = hand
            elif len(hand) == 3:  # Newer OpenCV version
                thresholded, hand_segment, _ = hand
